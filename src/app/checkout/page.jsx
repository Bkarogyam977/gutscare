  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white flex items-center justify-center px-4">
      <div className="max-w-md w-full text-center bg-white rounded-2xl shadow-lg p-8 border border-blue-100">
        ...
        <a href={paymentUrl} target="_blank" rel="noopener noreferrer" className="block w-full mb-3">
          <Button type="primary" size="large" block
            style={{ background: "#2563eb", borderColor: "#2563eb", fontWeight: 700, height: 52, fontSize: 16 }}>
            Open Payment Page →
          </Button>
        </a>
        <p className="text-xs text-stone-400 mb-6">Payment opens in a new tab — UPI · Debit/Credit Card · Net Banking</p>
        ...
        <Button size="small" type="text" onClick={onCancel} style={{ color: "#9ca3af", fontSize: 12 }}>
      ...
      <div className="max-w-md w-full text-center bg-white rounded-2xl shadow-lg p-8 border border-amber-100">
        ...
        {orderId && (
          <p className="text-stone-500 text-sm mb-1">
            Order ID: <span className="font-mono font-semibold text-amber-600">#{orderId}</span>
          </p>
        )}
        <p className="text-2xl font-bold text-amber-600 mt-2 mb-4">{formatPrice(total)}</p>
...
export default function CheckoutPage() {
  ...
  if (success?.payment_method === "online") {
    return (
      <PaymentPending
        ...
        onCancel={() => { clearCart(); router.push("/"); }}
      />
    );
  }
  ...
  if (success?.payment_method === "cod") {
    return (
      <OrderSuccess
        orderId={success.order_id}
        total={success.total}
        onContinue={() => { clearCart(); router.push("/"); }}
      />
    );
  }
  ...
  const handleSaveAddress = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        message.error(data.error || "Failed to save address. Please try again.");
        return;
      }
    ...
    <div className="min-h-screen bg-gradient-to-b from-amber-50 to-white">
      ...
      <div className="bg-white border-b border-amber-100 sticky top-0 z-10 shadow-sm">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4 flex items-center gap-3">
          ...
          <img src="https://bkarogyam.com/favicon.ico" alt="BK Arogyam"
            className="w-8 h-8 rounded-full object-cover" />
          <span className="font-bold text-stone-900">Checkout</span>

          {/* Step indicator */}
          <div className="ml-auto flex items-center gap-2 text-xs">
            <span className={`px-2.5 py-1 rounded-full font-semibold ${step === "address" ? "bg-amber-500 text-white" : "bg-green-100 text-green-700"}`}>
              1. Address
            </span>
            <span className="text-stone-300">→</span>
            <span className={`px-2.5 py-1 rounded-full font-semibold ${step === "payment" ? "bg-amber-500 text-white" : "bg-stone-100 text-stone-400"}`}>
              2. Payment
            </span>
          </div>
        </div>
              ...
              <Form form={form} layout="vertical" requiredMark={false}>
                <div className="bg-white rounded-2xl p-5 border border-stone-100 shadow-sm mb-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <Form.Item name="name" label="Full Name"
                      rules={[{ required: true, message: "Please enter your name" }]} className="mb-0">
                ...
                <div className="bg-white rounded-2xl p-5 border border-stone-100 shadow-sm mb-5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    <Form.Item name="building" label="Flat / House / Building No."
                      rules={[{ required: true, message: "Required" }]} className="mb-0">
            ...
            {step === "payment" && savedAddress && (
              <div className="space-y-4">
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <h2 className="font-semibold text-stone-800">Delivery Address Saved</h2>
                      </div>
                      <p className="text-sm text-stone-600 mt-1">
                        <span className="font-medium">{savedAddress.name}</span> · {savedAddress.mobile}
                      </p>
                      <p className="text-sm text-stone-500 mt-0.5">
                        {savedAddress.building}, {savedAddress.street}, {savedAddress.city}, {savedAddress.state} – {savedAddress.pincode}
                      </p>
                    </div>
                    <button
                      onClick={() => setStep("address")}
                      className="text-amber-600 hover:text-amber-700 text-xs flex items-center gap-1 shrink-0 mt-1"
                    >
                      <EditOutlined /> Edit
                    </button>
                  </div>
                ...
                <div className="bg-white rounded-2xl p-5 border border-stone-100 shadow-sm">
                  <h2 className="font-semibold text-stone-800 mb-4">Choose Payment Method</h2>
                  <div className="space-y-3">
                    ...
                    >
                      ...
                      {isCODBlocked && <WarningOutlined style={{ color: "#f59e0b", fontSize: 18 }} />}
                    </button>

                    {/* Online */}
                    <button
                      onClick={() => !orderLoading && setPaymentModalOpen(true)}
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          <Tag color="blue" style={{ fontSize: 10, lineHeight: "16px" }}>ICICI</Tag>
                        </div>
                        <p className="text-xs text-stone-500 mt-0.5">UPI, Debit/Credit Card, Net Banking</p>
                      </div>
                    </button>
                  </div>

                  {orderLoading && (
                    <p className="text-center text-sm text-stone-400 mt-4">Processing your order...</p>
                  )}
                </div>
              </div>
            )}
                ...
                {items.map((item) => (
                  <div key={item.id} className="flex gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-stone-700 line-clamp-2">{item.name}</p>
      ...
      >
        <div className="py-2">
          ...
          <Button
            ...
            style={{ background: "#2563eb", borderColor: "#2563eb", fontWeight: 700, height: 48 }}
          >
            Proceed to Payment
          </Button>
          {orderLoading && (
            <p className="text-center text-sm text-stone-400 mt-3">Processing your order...</p>
          )}
        </div>
      </Modal>
    </div>
